...
"""
Usage:
    error_analysis.py [options] MODEL_PATH (--conala-data | --staqc-data | --standard-dataset | --method2code-dataset) DATA_PATH OUT_FILE
...
Options:
    ...
    --conala-data                    The DATA_PATH is to the Conala dataset.
    --staqc-data                     The DATA_PATH is to the StaQC dataset.
...
def run(arguments):
    ...
    elif arguments['--conala-data']:
        assert len(test_data_dirs) == 1
        data = model_test.get_conala_dataset(test_data_dirs[0])
    elif arguments['--staqc-data']:
        assert len(test_data_dirs) == 1
        data = model_test.get_staqc_dataset(test_data_dirs[0])
    else:
